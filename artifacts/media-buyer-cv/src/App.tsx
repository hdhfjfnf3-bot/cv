...
function Counter({ to, suffix = "", label }: { to: number; suffix?: string; label: string }) {
  ...
  return (
    <div ref={ref} style={{ textAlign: "center" }}>
      <div className="shimmer-text" style={{ fontSize: "2.4rem", fontWeight: 900, lineHeight: 1 }}>
        {n}{suffix}
      </div>
      <div style={{ fontSize: "0.65rem", color: "rgba(255,255,255,0.35)", marginTop: 6, fontWeight: 600, letterSpacing: "0.1em", textTransform: "uppercase" }}>
        {label}
      </div>
    </div>
  );
...
/* ══════════════════════════════════════════════════
   TILT CARD (disabled on mobile for perf)
══════════════════════════════════════════════════ */
    ...
    >
      {!isMobile && (
        <div style={{
          ...
          background: `radial-gradient(circle at ${tilt.gx}% ${tilt.gy}%, rgba(255,255,255,0.07), transparent 60%)`,
        }} />
      )}
      {children}
    </div>
...
function SectionHeader({ num, title, sub }: { num: string; title: string; sub?: string }) {
  ...
  return (
    <Reveal style={{ marginBottom: 40, position: "relative" }}>
      ...
      <h2 style={{ fontSize: isMobile ? "2rem" : "2.8rem", fontWeight: 900, color: "#fff", lineHeight: 1.1, letterSpacing: "-0.02em", marginBottom: 8 }}>
        {title}
      </h2>
...
function Marquee() {
  ...
  return (
    <div style={{
      overflow: "hidden", borderTop: "1px solid rgba(232,201,109,0.07)",
      borderBottom: "1px solid rgba(232,201,109,0.07)", padding: "12px 0",
      ...
      <div className="marquee-track" style={{ gap: 28 }}>
        {items.map((item, i) => (
          <span key={i} style={{
            ...
            color: i % 3 === 0 ? "#e8c96d" : i % 3 === 1 ? "#a78bfa" : "rgba(255,255,255,0.3)",
            fontWeight: 600, fontSize: "0.78rem", letterSpacing: "0.08em", textTransform: "uppercase",
          }}>
            {item}
            <span style={{ color: "rgba(232,201,109,0.25)", fontSize: "0.5rem" }}>◆</span>
          </span>
        ))}
      </div>
    </div>
...
function ProfileHero({ scrollY }: { scrollY: any }) {
  ...
  const orbitDots = [
    { color: "#e8c96d", size: 8, r: 120, dur: 6, start: 0 },
    { color: "#7c3aed", size: 6, r: 130, dur: 9, start: 120 },
    { color: "#f43f5e", size: 5, r: 115, dur: 12, start: 240 },
    { color: "#06b6d4", size: 4, r: 135, dur: 15, start: 60 },
  ];
  ...
  return (
    <motion.div style={{ y: floatY, scale, display: "flex", alignItems: "center", justifyContent: "center", position: "relative" }}>
      ...
      {orbitDots.map((d, i) => (
        <motion.div key={i}
          style={{
            ...
            width: d.size, height: d.size, marginTop: -d.size / 2, marginLeft: -d.size / 2,
            borderRadius: "50%", background: d.color, boxShadow: `0 0 10px ${d.color}`,
          }}
          animate={{
            x: [Math.cos(d.start * Math.PI / 180) * d.r, Math.cos((d.start + 360) * Math.PI / 180) * d.r],
            y: [Math.sin(d.start * Math.PI / 180) * d.r, Math.sin((d.start + 360) * Math.PI / 180) * d.r],
          }}
      ...
      <motion.div
        ...
        transition={{ duration: 8, repeat: Infinity, ease: "linear" }}
        style={{
          position: "absolute", width: photoW + 40, height: photoH + 40,
          borderRadius: "60% 40% 50% 50% / 50% 50% 60% 40%",
          border: "2px solid transparent",
          background: "conic-gradient(from 0deg, #e8c96d, #7c3aed, #f43f5e, transparent, #e8c96d) border-box",
      ...
      <motion.div
        ...
        style={{
          position: "absolute", width: photoW + 68, height: photoH + 68,
      ...
      <motion.div
        ...
        style={{
          position: "absolute", width: photoW, height: photoH,
          background: "radial-gradient(ellipse, rgba(124,58,237,0.55), rgba(232,201,109,0.18), transparent)",
          filter: "blur(28px)", borderRadius: "60% 40% 50% 50% / 50% 50% 60% 40%",
        }}
      />
      ...
      <div style={{
        ...
        boxShadow: "0 0 50px rgba(124,58,237,0.45), 0 0 100px rgba(232,201,109,0.1)",
      }}>
        <img src="/profile.png" alt="Media Buyer"
          style={{ width: "100%", height: "100%", objectFit: "cover", objectPosition: "center top", filter: "brightness(1.05) contrast(1.08) saturate(1.1)" }} />
      ...
      {/* status badge */}
      <div style={{
        position: "absolute", bottom: 10, right: isMobile ? -6 : -14,
        background: "rgba(10,6,20,0.92)", border: "1px solid rgba(74,222,128,0.45)",
        borderRadius: 100, padding: "5px 12px", display: "flex", alignItems: "center", gap: 6,
...
function SidebarProfile() {
  return (
    <div style={{ display: "flex", flexDirection: "column", alignItems: "center", gap: 14 }}>
      ...
      >
        <div style={{
          width: 120, height: 148, borderRadius: "55% 45% 50% 50% / 50% 50% 55% 45%",
          overflow: "hidden", border: "2px solid rgba(232,201,109,0.45)",
          boxShadow: "0 0 36px rgba(124,58,237,0.5), 0 0 70px rgba(232,201,109,0.08)",
        }}>
      ...
      {[
        ...
        { icon: "📅", label: "Active Since", val: "April 2025" },
        { icon: "💼", label: "Work Type", val: "Freelancer" },
      ].map((item) => (
        <div key={item.label} style={{
          display: "flex", alignItems: "center", gap: 9, padding: "7px 11px", width: "100%",
      ...
      <div style={{ display: "flex", gap: 6, flexWrap: "wrap", justifyContent: "center" }}>
        {[{ label: "Meta", color: "#1877f2" }, { label: "TikTok", color: "#fe2c55" }, { label: "Snap", color: "#fffc00", text: "#000" }].map((p) => (
          <span key={p.label} style={{
            fontSize: "0.62rem", fontWeight: 800, padding: "3px 9px", borderRadius: 100,
            background: `${p.color}1a`, border: `1px solid ${p.color}44`, color: p.text || p.color, letterSpacing: "0.05em",
          }}>{p.label}</span>
        ))}
      </div>

      {/* CTA */}
      <motion.a href="#contact" className="glow-btn"
        style={{ display: "block", width: "100%", textAlign: "center", padding: "11px 0", borderRadius: 11, fontSize: "0.78rem", textDecoration: "none" }}
  ...
  return (
    ...
    >
      <TiltCard className="glass" style={{ padding: 24, height: "100%", cursor: "default" }}>
        <div style={{
          width: 50, height: 50, borderRadius: 14, background: accent,
          display: "flex", alignItems: "center", justifyContent: "center",
          marginBottom: 16, boxShadow: `0 0 20px ${accent.includes(",") ? "rgba(0,0,0,0.3)" : accent + "66"}`,
        }}>
          {icon}
        </div>
...
export default function App() {
  ...
  const springY = useSpring(scrollY, { stiffness: 80, damping: 20 });
  ...
  return (
    <div style={{ minHeight: "100vh", background: "#080612", position: "relative" }}>
      ...
      <motion.nav style={{
        ...
        backdropFilter: "blur(18px)", background: navBg,
        borderBottom: "1px solid", borderColor: navBorder,
      }} initial={{ y: -70 }} animate={{ y: 0 }} transition={{ duration: 0.7, ease: "easeOut" }}>
        <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
          <div style={{ width: 9, height: 9, borderRadius: "50%", background: "linear-gradient(135deg, #7c3aed, #e8c96d)", boxShadow: "0 0 10px #7c3aed" }} />
          ...
          <div style={{ display: "flex", gap: 28 }}>
            {NAV_LINKS.map((item) => (
              <a key={item} href={`#${item.toLowerCase()}`}
                style={{ color: "rgba(255,255,255,0.4)", textDecoration: "none", fontSize: "0.78rem", fontWeight: 600, letterSpacing: "0.07em", textTransform: "uppercase", transition: "color 0.2s" }}
      ...
      {isMobile && navOpen && (
        <motion.div initial={{ opacity: 0, y: -10 }} animate={{ opacity: 1, y: 0 }}
          style={{
            ...
            background: "rgba(8,6,18,0.97)", backdropFilter: "blur(20px)",
            borderBottom: "1px solid rgba(232,201,109,0.1)", padding: "16px 20px",
          ...
          {NAV_LINKS.map((item) => (
            <a key={item} href={`#${item.toLowerCase()}`} onClick={() => setNavOpen(false)}
              style={{
                ...
                fontSize: "0.88rem", fontWeight: 600, padding: "12px 0",
                borderBottom: "1px solid rgba(255,255,255,0.05)", letterSpacing: "0.04em",
              }}>
              {item}
            </a>
          ))}
      ...
      <section style={{
        ...
        padding: isMobile ? "100px 20px 60px" : "110px 6% 60px",
        ...
        <div style={{
          ...
          {/* text side */}
          <motion.div style={{ y: heroY, opacity: heroOpacity, order: isMobile ? 2 : 1 }}
            initial={{ opacity: 0, x: -40 }} animate={{ opacity: 1, x: 0 }} transition={{ duration: 0.9, ease: "easeOut" }}>
            {/* badge */}
            <motion.div animate={{ scale: [1, 1.02, 1] }} transition={{ duration: 3.5, repeat: Infinity }}
              style={{
                ...
                border: "1px solid rgba(232,201,109,0.28)", background: "rgba(232,201,109,0.05)",
              }}>
              <span style={{ width: 7, height: 7, borderRadius: "50%", background: "#4ade80", display: "inline-block", boxShadow: "0 0 8px #4ade80" }} />
            ...
            <h1 style={{
              fontSize: isMobile ? "3rem" : "clamp(3.5rem, 6.5vw, 5.5rem)",
              fontWeight: 900, color: "#fff", lineHeight: 1.05,
              letterSpacing: "-0.03em", marginBottom: 18,
            }}>
              Digital<br />
              <span className="gold-text">Media</span><br />
              <span style={{ color: "rgba(255,255,255,0.22)", fontSize: "55%" }}>Buyer</span>
            </h1>
            ...
            <div style={{ display: "flex", gap: 12, flexWrap: "wrap" }}>
              <motion.a href="#contact" className="glow-btn magnetic-btn"
                style={{ padding: "13px 28px", borderRadius: 11, fontSize: "0.85rem", textDecoration: "none", display: "inline-block" }}
                whileHover={{ scale: 1.04 }} whileTap={{ scale: 0.97 }}>
                Contact Me Now ✦
              </motion.a>
              <motion.a href="#skills" className="outline-btn"
                style={{ padding: "13px 28px", borderRadius: 11, fontSize: "0.85rem", textDecoration: "none", display: "inline-block" }}
                whileHover={{ scale: 1.04 }} whileTap={{ scale: 0.97 }}>
                View Skills
              </motion.a>
      ...
      <div style={{
        ...
        padding: isMobile ? "0 20px 80px" : "0 6% 100px",
        display: "grid",
        ...
        <div id="skills" style={{ display: "flex", flexDirection: "column", gap: isMobile ? 60 : 80 }}>
            ...
            <Reveal style={{ marginTop: 4 }}>
              <div style={{ display: "flex", flexWrap: "wrap", gap: 8 }}>
                {["Meta Ads Manager", "TikTok Business", "Snap Ads", "A/B Testing", "ROAS", "Retargeting", "Lookalike Audiences", "CRO", "Data Analysis", "Pixel Setup", "Campaign Scaling", "Creative Testing", "Budget Management", "eCommerce"].map((tag, i) => (
                  <motion.span key={tag}
                    style={{ padding: "5px 12px", borderRadius: 100, fontSize: "0.7rem", fontWeight: 600, background: "rgba(124,58,237,0.08)", border: "1px solid rgba(124,58,237,0.2)", color: "rgba(167,139,250,0.8)", cursor: "default" }}
                    initial={{ opacity: 0, scale: 0.8 }}
                    whileInView={{ opacity: 1, scale: 1 }} viewport={{ once: true }}
                    transition={{ delay: i * 0.04 }}
                    whileHover={{ background: "rgba(124,58,237,0.2)", borderColor: "rgba(124,58,237,0.5)", scale: 1.06 }}>
                    {tag}
                  </motion.span>
                ))}
              </div>
                ...
                <div className="glass" style={{ padding: isMobile ? 22 : 32 }}>
                  <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", flexWrap: "wrap", gap: 10, marginBottom: 22 }}>
                    <div>
                      ...
                      <div style={{ color: "#e8c96d", fontWeight: 700, fontSize: "0.85rem" }}>Freelance — Egypt</div>
                    </div>
                    <div className="glass-purple" style={{ padding: "7px 16px", borderRadius: 100, fontSize: "0.74rem", color: "#a78bfa", fontWeight: 700, whiteSpace: "nowrap" }}>
                      April 2025 — Present
                    </div>
                  </div>
                  <div style={{ display: "grid", gridTemplateColumns: isMobile ? "1fr" : "1fr 1fr", gap: 10 }}>
                    {[
                      "Managed ad campaigns on Meta (Facebook & Instagram), TikTok & Snapchat",
                      "Tested multiple Creatives, Audiences & Offers to find winning combinations",
                      "Daily campaign monitoring with data-driven decision making",
                      "Optimized Conversion Rate (CRO) and reduced cost per result",
                      "Scaled successful campaigns while maintaining strong ROAS",
                      "Set up and managed tracking tools for accurate data collection",
                      "Delivered clear performance reports with actionable recommendations",
                    ].map((item, i) => (
                      <motion.div key={i}
                        style={{ display: "flex", alignItems: "flex-start", gap: 9, padding: "10px 12px", borderRadius: 10, background: "rgba(124,58,237,0.05)", border: "1px solid rgba(124,58,237,0.1)", fontSize: "0.78rem", color: "rgba(255,255,255,0.55)", lineHeight: 1.55 }}
                        initial={{ opacity: 0, x: -16 }} whileInView={{ opacity: 1, x: 0 }} viewport={{ once: true }}
                        transition={{ delay: i * 0.08 }}
                        whileHover={{ background: "rgba(124,58,237,0.12)", color: "rgba(255,255,255,0.82)" }}>
                        <span style={{ color: "#7c3aed", marginTop: 2, flexShrink: 0, fontSize: "0.65rem" }}>◆</span>
                        {item}
                      </motion.div>
                    ))}
                  </div>
          ...
          <section id="platforms">
            ...
            <div style={{ display: "grid", gridTemplateColumns: isMobile ? "1fr" : "repeat(3, 1fr)", gap: 16 }}>
              <PlatCard title="Meta Ads" accent="linear-gradient(135deg,#1877f2,#0c52b3)" delay={0.0}
                ...
                skills={["Facebook & Instagram Ads", "Custom Audiences", "Lookalike Audiences", "Retargeting Campaigns", "Pixel Setup & Tracking", "Campaign Scaling"]} />
              <PlatCard title="TikTok Ads" accent="linear-gradient(135deg,#010101,#2a2a2a)" delay={0.1}
                icon={<svg viewBox="0 0 24 24" fill="white" width="24" height="24"><path d="M19.59 6.69a4.83 4.83 0 01-3.77-4.25V2h-3.45v13.67a2.89 2.89 0 01-2.88 2.5 2.89 2.89 0 01-2.89-2.89 2.89 2.89 0 012.89-2.89c.28 0 .54.04.79.1V9.01a6.27 6.27 0 00-.79-.05 6.34 6.34 0 00-6.34 6.34 6.34 6.34 0 006.34 6.34 6.34 6.34 0 006.33-6.34V8.69a8.17 8.17 0 004.78 1.52V6.75a4.85 4.85 0 01-1.01-.06z" /></svg>}
                skills={["In-Feed Ads", "Spark Ads", "TopView Campaigns", "Creative Testing", "TikTok Pixel Setup", "Gen-Z Targeting"]} />
              <PlatCard title="Snapchat Ads" accent="linear-gradient(135deg,#fffc00,#e6e300)" delay={0.2}
                icon={<svg viewBox="0 0 24 24" fill="#000" width="24" height="24"><path d="M12.206.793c.99 0 4.347.276 5.93 3.821.529 1.193.403 3.219.299 4.847l-.003.06c-.012.18-.022.345-.03.51.075.045.203.09.401.09.3 0 .677-.084 1.03-.189.097-.028.218-.044.316-.037.132.012.22.075.22.15.001.082-.1.176-.22.228-.18.077-.409.12-.62.12-.12 0-.24-.012-.36-.035.055.183.106.37.145.569.018.09.02.18.016.27-.004.15-.063.3-.16.413-.1.124-.253.211-.413.247-.088.02-.16.032-.228.032-.1 0-.177-.024-.26-.065-.225-.11-.45-.227-.82-.274-.18-.024-.359-.035-.54-.035-.358 0-.713.065-1.058.19-.45.16-.9.333-1.412.333-.512 0-.96-.173-1.412-.333-.345-.125-.7-.19-1.058-.19-.181 0-.36.011-.54.035-.37.047-.595.164-.82.274-.083.041-.16.065-.26.065-.068 0-.14-.012-.228-.032-.16-.036-.313-.123-.413-.247-.097-.113-.156-.263-.16-.413-.004-.09-.002-.18.016-.27.039-.199.09-.386.145-.569-.12.023-.24.035-.36.035-.211 0-.44-.043-.62-.12-.12-.052-.221-.146-.22-.228 0-.075.088-.138.22-.15.098-.007.219.009.316.037.353.105.73.189 1.03.189.198 0 .326-.045.4-.09-.008-.165-.017-.33-.03-.51l-.002-.06c-.104-1.628-.23-3.654.299-4.847C7.853 1.069 11.215.793 12.206.793z" /></svg>}
                skills={["Story Ads", "Collection Ads", "Dynamic Ads", "AR Lens Campaigns", "Snap Pixel Setup", "Youth Targeting"]} />
            </div>
          </section>
          ...
          <section>
            ...
            <div style={{ display: "grid", gridTemplateColumns: isMobile ? "1fr" : "1fr 1fr", gap: 14 }}>
              {[
                { emoji: "🎯", title: "Campaign Management", desc: "Full management of your ad campaigns on Meta, TikTok & Snapchat with maximum efficiency.", accent: "#7c3aed" },
                { emoji: "📊", title: "Data Analysis & Reports", desc: "Daily, weekly and monthly reports with clear insights and actionable recommendations.", accent: "#e8c96d" },
                { emoji: "🚀", title: "Campaign Scaling", desc: "Growing winning campaigns gradually while protecting your ROAS and performance targets.", accent: "#f43f5e" },
                { emoji: "🔬", title: "A/B Testing", desc: "Systematic testing of Creatives, Audiences & Offers to find the best-performing combo.", accent: "#06b6d4" },
                { emoji: "📡", title: "Tracking Setup", desc: "Accurate Pixel & tracking configuration to ensure clean data and confident decisions.", accent: "#7c3aed" },
                { emoji: "💡", title: "Marketing Consulting", desc: "Full ad strategy tailored to your product type and target audience.", accent: "#e8c96d" },
              ].map((s, i) => (
            ...
            <Reveal>
              <div className="glass-dark gradient-border" style={{ padding: isMobile ? 26 : 38, borderRadius: 20, position: "relative", overflow: "hidden" }}>
                ...
                <div style={{ display: "grid", gridTemplateColumns: isMobile ? "1fr" : "1fr 1fr", gap: 28, position: "relative", zIndex: 1 }}>
                  <div>
                    <p style={{ fontSize: "0.95rem", color: "rgba(255,255,255,0.5)", lineHeight: 1.85, marginBottom: 24, fontWeight: 300 }}>
                      I don't work on gut feelings — I work on data. Every decision I make is backed by real numbers from live campaigns, monitored daily.
                    </p>
                    {[
                      { label: "Data-driven decisions, not guesswork", color: "#7c3aed" },
                      { label: "Daily monitoring & continuous optimization", color: "#e8c96d" },
                      { label: "Full transparency in results", color: "#f43f5e" },
                      { label: "Focus on real ROI, not vanity metrics", color: "#06b6d4" },
                    ].map((p, i) => (
                  ...
                  {!isMobile && (
                    <div style={{ display: "flex", alignItems: "center", justifyContent: "center" }}>
                      <div style={{ position: "relative", width: 160, height: 160 }}>
                        ...
                        {["🎯", "💰", "📊", "🚀"].map((icon, i) => (
                          <motion.div key={i}
                            style={{ position: "absolute", top: "50%", left: "50%", width: 34, height: 34, marginTop: -17, marginLeft: -17, borderRadius: "50%", background: "rgba(255,255,255,0.05)", border: "1px solid rgba(255,255,255,0.1)", display: "flex", alignItems: "center", justifyContent: "center", fontSize: "0.9rem" }}
                            animate={{ x: Math.cos(i * Math.PI / 2) * 72, y: Math.sin(i * Math.PI / 2) * 72, rotate: [0, 360] }}
                            transition={{ x: { duration: 8, repeat: Infinity, ease: "linear" }, y: { duration: 8, repeat: Infinity, ease: "linear" }, rotate: { duration: 8, repeat: Infinity, ease: "linear" } }}>
                            {icon}
                          </motion.div>
                        ))}
                      </div>
          ...
          <section id="contact">
            ...
            <Reveal>
              <div style={{ display: "grid", gridTemplateColumns: isMobile ? "1fr" : "repeat(3, 1fr)", gap: 12, marginBottom: 16 }}>
                {[
                  { icon: "💬", label: "WhatsApp", val: "Chat Now", color: "#25D366", href: "https://wa.me/201234567890" },
                  { icon: "📧", label: "Email", val: "media@buyer.pro", color: "#e8c96d", href: "mailto:media@buyer.pro" },
                  { icon: "📱", label: "Telegram", val: "@mediabuyerpro", color: "#2AABEE", href: "https://t.me/mediabuyerpro" },
                ].map((c, i) => (
              ...
              <motion.div className="glass-purple"
                style={{ padding: isMobile ? 28 : 40, textAlign: "center", borderRadius: 20, position: "relative", overflow: "hidden" }}
                initial={{ opacity: 0, scale: 0.96 }} whileInView={{ opacity: 1, scale: 1 }} viewport={{ once: true }}
                whileHover={{ boxShadow: "0 0 60px rgba(124,58,237,0.25)" }}>
                ...
                <div style={{ position: "relative", zIndex: 1 }}>
                  ...
                  <motion.a href="https://wa.me/201234567890" target="_blank" rel="noopener noreferrer"
                    className="glow-btn"
